...
const getAll = async () => {
  const response = await axios.get<Dashboard[]>(`${API_URL}/boards`);
  return response.data;
};

const getById = async (id: number) => {
  const response = await axios.get<Dashboard>(`${API_URL}/boards/${id}`);
  return response.data;
};

const create = async (data: { title: string }) => {
  const response = await axios.post<Dashboard>(`${API_URL}/boards`, data);
  return response.data;
};
